...
public:
    ...
    // virtual void toline_res(VertexT* v, BufferedWriter& writer) = 0; //this is what user specifies!!!!!!
    ...
    void dump_partition(const char* outpath)
    {
        ...
        // hdfsFS fs_r = getHdfsFS();
        // string outpath_r = "/user/lyuan/cqh/ppr/arabicOutput_r";
        // BufferedWriter* writer_r = new BufferedWriter(outpath_r.c_str(), fs_r, _my_rank);

        // for (VertexIter it = vertexes.begin(); it != vertexes.end(); it++) {
        //     writer_r->check();
        //     toline_res(*it, *writer_r);
        // }
        // delete writer_r;
        // hdfsDisconnect(fs_r);
    }
    ...
    void run(const WorkerParams& params)
    {
        ...
        sort(vertexes.begin(), vertexes.end(), 
            [](const VertexT* a, const VertexT* b) {
              return a->id < b->id; 
            });
        ...
        while (true) {
            ...
            vector<VertexT*>& to_add = message_buffer->sync_messages(params.sz_error);
            agg_sync();
            ...
            StopTimer(8);
            if (_my_rank == MASTER_RANK) {
                cout << "Superstep " << global_step_num << " done. Time elapsed: " << get_timer(8) << " seconds" << endl;
        ...
        //PrintTimer("-- Transfer Key Time", TRANSFER_KEY_TIMER);
        //PrintTimer("-- Transfer Val Time", TRANSFER_VAL_TIMER);
        PrintTimer("- Compression Time", COMPRESSION_TIMER);
        PrintTimer("- Decompression Time", DECOMPRESSION_TIMER);
