...
//==========================================
//Default Hash Function for Local Index ====
template <class KeyT>
class DefaultHashIndex {
public:
    inline int operator()(KeyT key)
    {
        if (key >= 1)
            return (key - 1) / _num_workers;
        else
            return (-key + 1) / _num_workers;
    }
};
...
//     {
//         if (key >= 0)
//             return (key - 1) % (2 * _num_workers) >= _num_workers ? \
//                 2 * _num_workers - 1 - (key - 1) % (2 * _num_workers) : (key - 1) % (2 * _num_workers);
//         else
//             return (-key) % (2 * _num_workers);
//     }
// };
// //==========================================
// //Default Hash Function for Local Index ====
...
//         if (key >= 1)
//             return (key - 1) % (2 * _num_workers) >= _num_workers ? \
//                 (key - 1) / (2 * _num_workers) * 2 + 1 : (key - 1) / (2 * _num_workers) * 2;
//         else
//             return (-key + 1) / _num_workers;
//     }
// };
...
template <class KeyT, class ValueT, class MessageT, class HashT = DefaultHash<KeyT>, \
        class HashIdxT = DefaultHashIndex<KeyT> >
class Vertex {
public:
    KeyT id;
